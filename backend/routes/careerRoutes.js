...
// POST /api/career/insights — Career Insights (NO file upload needed)
// Body: { role, experience, location, skills }
router.post("/insights", authMiddleware, careerInsightsController);

// POST /api/career-dna/analyze-dna — Career DNA (requires resume upload)
router.post("/analyze-dna", authMiddleware, upload.single("resume"), analyzeCareerDNAController);

// Keep old /insights alias for Career DNA backward compatibility
router.post("/dna-insights", authMiddleware, upload.single("resume"), analyzeCareerDNAController);
